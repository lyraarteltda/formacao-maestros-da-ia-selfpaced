...
const VideoCard = ({ testimonial, index }: { testimonial: VideoTestimonial; index: number }) => {
  return (
    <motion.div
      ...
      <div className="relative bg-card/50 backdrop-blur-sm border border-border/50 rounded-2xl overflow-hidden transition-all duration-300 hover:border-primary/50 hover:shadow-[0_10px_40px_-15px_hsl(var(--primary)/0.3)] hover:-translate-y-1">
        ...
        <div className="relative aspect-[9/16] bg-gradient-to-br from-muted/80 to-muted/40 overflow-hidden">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <span className="text-xs text-muted-foreground/60 mb-2">
              Upload: {testimonial.name.toLowerCase().replace(' ', '_')}.mp4
            </span>
          </div>
          ...
          {!testimonial.isPlaceholder && (
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="relative">
                ...
                {/* Play button */}
                <div className="relative w-16 h-16 bg-primary/90 rounded-full flex items-center justify-center shadow-lg group-hover:bg-primary group-hover:scale-110 transition-all duration-300 cursor-pointer">
